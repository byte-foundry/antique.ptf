		...
		1:
			...
			nodes:
				...
				4:
					...
					y: capHeight
					dirIn: Utils.lineAngle( contours[1].nodes[4].point, contours[1].nodes[3].point ) - ( 50 / 180 * Math.PI )
					expand: Object({
						width: ( 28 / 54 ) * thickness * contrast
						angle: Utils.lineAngle( contours[1].nodes[4].point, contours[1].nodes[3].point ) + Math.PI / 2 - ( 40 / 180 * Math.PI )
						distr: 0
					})
			transformOrigin: Array( (contours[1].nodes[0].expandedTo[0].x + contours[1].nodes[4].expandedTo[1].x ) / 2, (contours[1].nodes[0].expandedTo[0].y + contours[1].nodes[4].expandedTo[0].y ) / 2 )
			transforms: Array(
				['rotate', 180 + 'deg']
			)
